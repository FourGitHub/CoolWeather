...
public class PCCUtil {
    ...
    /**
     * 解析和处理服务器返回的省级数据，并保存到数据库表中，以便于下一次直接从数据库中读取数据，而不是反复发送网络请求
     */
    ...
    /**
     * 将返回的JSON数据解析成MyWeatherBean实体类
     */
    public static MyWeatherBean handleWeatherResponse(String response) {
        try {
            JSONObject jsonObject = new JSONObject(response);
            JSONArray jsonArray = jsonObject.getJSONArray("HeWeather6");
            String weatherContent = jsonArray.getJSONObject(0).toString();
           return new Gson().fromJson(weatherContent, MyWeatherBean.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
